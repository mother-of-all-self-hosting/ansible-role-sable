...
#
# Tags look like `v<Sable version>-<release>`:
#
# - if defaults/main.yml points at a Sable version that has never been
#   released, the release counter restarts at 0 (`v1.21.0-0`)
# - otherwise the counter is incremented (`v1.21.0-1`), but only if something
...
version="$(sed -nE 's|^sable_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Sable version from $defaults_path"
	exit 1
fi
